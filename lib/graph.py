...
def solution_plot(dfunc, c, sig, xc, yc, dims, base_level=0., mask=None, 
    ...
    plt.subplot(1,3,1)
    ax = plt.gca()
    im = ax.imshow(f, vmin=0., vmax=1., cmap=plt.cm.gray)
    plt.title('Original')
    plt.axis('off')
    ...
    plt.subplot(1,3,2)
    ax = plt.gca()
    im = ax.imshow(u, vmin=0., vmax=1., cmap=plt.cm.gray)
    plt.title('Solution')
    plt.axis('off')
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    ...
    plt.subplot(1,3,3)
    ax = plt.gca()
    im = ax.imshow(res, vmin=0., vmax=1., cmap=plt.cm.gray)
    plt.title('Residual')
    plt.axis('off')
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
...
def points_plot(data, center_points=None, collocation_points=None, boundary_points=None, title=None):
    ...
    if (center_points is not None) and (collocation_points is None):
        ...
        ax2.imshow(data, cmap=plt.cm.gray)
